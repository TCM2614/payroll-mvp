/**
 * Curated catalogue of UK annual gross salaries that get a first-class
 * `/salary/[slug]-after-tax` landing page.
 *
 * The list is intentionally *curated*, not mechanical. Rules of thumb:
 *
 *   - £1k intervals £18k → £60k  (highest UK search volume)
 *   - £5k intervals £65k → £100k
 *   - Anchor jumps at £100k / £110k / £120k / £125k / £130k / £150k for
 *     the £100k tax trap.
 *   - Flag `landmark: true` on salaries used as anchors in the internal
 *     salary graph and marketing calendar.
 *
 * Extending this list requires only a one-line change — the salary page
 * `/salary/[slug]-after-tax` uses this catalogue to pre-generate pages at
...
export const SALARY_CATALOG: SalaryCatalogEntry[] = [
  ...
  .map((entry) => {
    const landmark = [
      18_000, 20_000, 25_000, 30_000, 35_000, 40_000, 45_000, 50_000, 55_000,
      60_000, 70_000, 75_000, 80_000, 90_000, 100_000, 125_000, 150_000,
    ].includes(entry.salary);
    return { ...entry, landmark };
  })
  .filter(
    (entry, i, arr) => arr.findIndex((e) => e.salary === entry.salary) === i,
  )
  .sort((a, b) => a.salary - b.salary);

/**
 * Landmark salaries used for cross-linking and the marketing calendar.
 * These are the salaries most likely to be searched independently and
 * also the salaries most useful for pay-rise comparisons.
 */
